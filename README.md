...
| **JDK/Spring** | JDK 17 (LTS), Spring Boot 3.x | 장기 지원(LTS) 버전 채택 및 최신 Spring 버전 사용으로 안정성과 생산성을 확보함. |
...
| `/api/vats` | GET | 부가세 계산 결과를 조회합니다. (ADMIN은 전체, MANAGER는 권한 사업장만 조회) | `year`, `halfType` (Query) | ADMIN / MANAGER |
...
### B. 수집기(CollectorClient) 비동기 처리

수집 요청 API가 호출되면, `CollectorClient`는 다음과 같이 처리합니다.
